...
main()
{
	...
	for(int i = 0; i < n; i++)
	{
		if(matrix[i][i] == 0)
		{
			fprintf(out,"Áåñêîíå÷íî ìíîãî ðåøåíèé.");
			return 0;
		}
	}
	...
	fprintf(out,"Êîëè÷åñòâî èòåðàöèé: %d\n",chek);
...
void vvod()
{
	...
	for(int i = 0; i < 10 ; i++)
	{
		fscanf(in,"%s", s);fscanf(in,"%lf", &f[i]);//ñâîáîäíûå êî-òû
	}
	...
	for(int i = 0; i < 10 ; i++)
	{
		fscanf(in,"%lf", &y[i]);//íà÷àëüíûå çíà÷åíèÿ
		y_old[i] = y[i];
	}
...
double find_root()
{
	for(int i = 0; i < n ;i++)
	{
		...
		y_old[i] = y[i];
		y[i] = (f[i] - yq)/matrix[i][i]; //ñîçäàì ïðåìåííóþ äëÿ ýòîãî ýëåìåíòà
	}
	
	double max = 0;
	for(int i = 0; i < n;i++) //ïðîâåðêà, îòáîð íàèáîëüøåé íåâÿçêè
	{
